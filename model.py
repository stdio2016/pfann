...
class SeparableConv2d(Module):
    def __init__(self, i, o, k, s, in_F, in_T, fuller=False):
...
class MyF(Module):
    def __init__(self, d, h, u, in_F, in_T, fuller=False):
        ...
        for i in range(8):
            k = 3
            s = 2
            convs.append(SeparableConv2d(channels[i], channels[i+1], k, s, in_F, in_T, fuller=fuller))
            in_F = (in_F-1)//s + 1
            in_T = (in_T-1)//s + 1
...
class FpNetwork(Module):
    def __init__(self, d, h, u, F, T, fuller=False):
